<template>
  <div>
    <page-header> Productos </page-header>

    <div class="flex space-x-2">
      <Link class="btn btn-primary" :href="route('products.create')">
        <span>Crear</span>
      </Link>
    </div>
...
export default defineComponent({
  ...
  setup(props) {
    const load = (newParams) => {
      ...
      Inertia.get(`/products?${urlQuery}`, [], {
        preserveState: true,
      });
    };
    const deleteRow = (rowId) => {
      if (confirm("¿Estás seguro de que quieres eliminar?")) {
        Inertia.delete(route("products.destroy", rowId));
      }
    };
